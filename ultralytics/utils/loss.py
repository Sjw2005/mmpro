...
from .metrics import bbox_iou, bbox_inner_iou, probiou
...
class InnerBboxLoss(nn.Module):
    """使用Inner-IoU计算bbox损失，增强小目标梯度信号。"""

    def __init__(self, reg_max=16, inner_ratio=0.7):
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask],
                             xywh=False, CIoU=True, ratio=self.inner_ratio)
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=15):  # model must be de-paralleled
        ...
        if cls_weight_str:
            ...
            self.cls_weights = torch.tensor(weights, device=device)
            print(f"  [ClassWeight] 启用类别加权BCE: {dict(zip(range(len(weights)), weights))}")
        else:
