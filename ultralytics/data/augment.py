...
# class RandomHSV:  #对融合后的6通道图像进行拆开，分别对RGB和红外图像进行HSV变换，再合并
#     #H=色调（比如红、绿、蓝），S=饱和度（颜色浓淡），V=明度（明暗）
#     """
...
#         """
#         img = labels["img"]
#         #判断：如果hgain、sgain、vgain中有一个不是0，说明需要做HSV调整（有一个要调整就执行下面的代码）
...
#             ## -------------- 处理“红外+可见光”多通道图像的情况 --------------
#             # 判断：如果图像的最后一个维度（通道数）大于5，说明是多通道图像（比如红外3通道+可见光3通道=6通道）
#             if img.shape[-1] > 5:
#                 hue, sat, val = cv2.split(cv2.cvtColor(img[..., -3:], cv2.COLOR_BGR2HSV))   # 处理可见光部分
#                 hue_, sat_, val_ = cv2.split(cv2.cvtColor(img[..., :-3], cv2.COLOR_BGR2HSV))   # 处理红外部分
...
class RandomHSV:
    """
    Random HSV augmentation for RGB images.
    For 6-channel fused images (RGB+IR), only RGB gets HSV; IR gets mild intensity scaling.
    """
    def __init__(self, hgain=0.5, sgain=0.5, vgain=0.5, order="rgb_ir", ir_vgain=None) -> None:
        """
        Args:
            hgain, sgain, vgain: RGB HSV jitter gains (same meaning as Ultralytics).
            order: channel order for 6-channel images:
                   - "rgb_ir": img[..., :3] is RGB, img[..., 3:6] is IR (matches your Multiin)
                   - "ir_rgb": img[..., :3] is IR,  img[..., 3:6] is RGB
            ir_vgain: optional IR intensity gain limit. If None, it will be derived from vgain (smaller).
                      Recommended to keep IR gain mild, e.g. 0.1 ~ 0.2.
        """
        ...
        # IR gain should be much smaller than RGB vgain to avoid destroying thermal statistics
        if ir_vgain is None:
            self.ir_vgain = float(min(0.2, self.vgain * 0.5))  # e.g. vgain=0.5 -> ir_vgain=0.2
        else:
            self.ir_vgain = float(ir_vgain)

    def _augment_rgb_hsv(self, rgb, r, dtype):
        """Apply HSV LUT on a BGR RGB image."""
        x = np.arange(0, 256, dtype=r.dtype)
        lut_hue = ((x * r[0]) % 180).astype(dtype)
        lut_sat = np.clip(x * r[1], 0, 255).astype(dtype)
        lut_val = np.clip(x * r[2], 0, 255).astype(dtype)
        ...
        im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
        rgb_out = cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR)
        return rgb_out

    def _augment_ir_intensity(self, ir, ir_gain, dtype):
        """
        IR: do NOT apply HSV.
        Apply mild intensity scaling (and clip). Works for 3-channel pseudo-IR or real 3-channel IR.
        """
        ir_f = ir.astype(np.float32) * float(ir_gain)
        ir_out = np.clip(ir_f, 0, 255).astype(dtype)
        return ir_out
    ...
    def __call__(self, labels):
        ...
        # Sample random gains for RGB HSV
        r = np.random.uniform(-1, 1, 3).astype(np.float32) * np.array([self.hgain, self.sgain, self.vgain], np.float32) + 1.0
        dtype = img.dtype

        # --- 6-channel case: RGB+IR ---
        if img.ndim == 3 and img.shape[-1] == 6:
            if self.order == "rgb_ir":
                rgb = img[..., :3]
                ir = img[..., 3:6]
            elif self.order == "ir_rgb":
                ir = img[..., :3]
                rgb = img[..., 3:6]
            else:
                raise ValueError(f"Unknown channel order: {self.order}. Use 'rgb_ir' or 'ir_rgb'.")

            # RGB HSV augmentation
            rgb_aug = self._augment_rgb_hsv(rgb, r, dtype)

            # IR mild intensity scaling (derived from vgain, but much smaller)
            ir_r = float(np.random.uniform(-1, 1) * self.ir_vgain + 1.0)  # e.g. ir_vgain=0.2 -> [0.8, 1.2]
            ir_aug = self._augment_ir_intensity(ir, ir_r, dtype)
        ...
        # --- Standard 3-channel RGB case ---
        if img.ndim == 3 and img.shape[-1] == 3:
            rgb_aug = self._augment_rgb_hsv(img, r, dtype)
            labels["img"] = rgb_aug
            return labels

        # --- Other channel counts: do nothing (safe fallback) ---
        return labels
