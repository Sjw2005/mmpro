...
class SFS(nn.Module):
    def __init__(
        ...
        if filter == "FrFT":
            self.FPU = FourierFPU(in_channels // 2, in_channels, order)
        elif filter == "FrGT":
            self.FPU = GaborFPU(in_channels // 2, in_channels, order)

        self.PWC_o = Conv(in_channels, in_channels, 1)
        self.advavg = nn.AdaptiveAvgPool2d(1)

    def forward(self, data):
        x, y = data
        x_spa = self.SPU(self.PWC0(x))
        x_fre = self.FPU(self.PWC1(y))
